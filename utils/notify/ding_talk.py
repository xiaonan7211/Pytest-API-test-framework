...
class DingTalkSendMsg:
    ...
    def send_ding_notification(self):
        ...
        if self.metrics.failed + self.metrics.broken > 0:
            is_at_all = False
        text = f"#### {config.project_name}自动化测试机器人自动推送Gymera测试报告  " \
               ...
               f")\n" \
               f" > ###### 测试报告 [详情](http://{get_host_ip()}:8000) \n"
        # f" > ###### 测试报告 [详情]({get_host_ip()}:8000/index.html) \n"
...
if __name__ == '__main__':
    ...
    print(f" > ###### 测试报告 [详情]({get_host_ip()}:8000/index.html")