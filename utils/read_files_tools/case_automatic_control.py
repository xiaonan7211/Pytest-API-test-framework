...
class TestCaseAutomaticGeneration:
    ...
    @staticmethod
    def pytest_mark(case_data: Dict, file_path) -> Text:
        """
        返回pytest用例标记
        """
        try:
            return case_data["case_common"]["mark"]
        except KeyError as exc:
            raise ValueNotFoundError(TestCaseAutomaticGeneration.error_message(
                param_name="mark",
                file_path=file_path
    ...
    @staticmethod
    def pytest_order(case_data: Dict) -> Text:
        """
        兼容需要排序的用例和不需要排序的用例
        """

        order = case_data.get("case_common").get("order")
        if order:
            return order
    ...
    def case_ids(test_case):
        ...
        for k, v in test_case.items():
            if k != "case_common" and v["is_run"] != False:
                ids.append(k)
        return ids
        ...
        for file in file_path:
            ...
            if 'proxy_data.yaml' not in file:
                ...
                write_testcase_file(
                    ...
                    mark=self.pytest_mark(case_data=yaml_case_process, file_path=file),
                    # order=self.pytest_order(case_data=yaml_case_process)
                    )
