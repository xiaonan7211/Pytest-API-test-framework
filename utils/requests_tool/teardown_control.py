...
class TearDownHandler:
    ...
    def teardown_http_requests(cls, teardown_case: Dict) -> "ResponseData":
        ...
        # 后置请求修改用例是否执行（被依赖的用例一定会执行，但是默认不执行）
        test_case["is_run"] = True
        req_con = RequestControl(test_case)
        if test_case["dependence_case"] == True:
            from utils.requests_tool.dependent_case import DependentCase
            DependentCase(req_con._RequestControl__yaml_case).get_dependent_data()
        res = req_con.http_request(
