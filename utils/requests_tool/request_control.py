...
class RequestControl:
    ...
    @staticmethod
    def transferred_meaning(
            ignore_data: Union[Text, List]
    ):
        """
        把list集合序列化为 "[\"string\"]"  json串
        """
        if isinstance(ignore_data, str):
            ignore_data_list = list()
            ignore_data_list.append(ignore_data)
            ignore_data = json.dumps(ignore_data_list, ensure_ascii=False).encode("utf-8")
        elif isinstance(ignore_data, (list, dict)):
            ignore_data = json.dumps(ignore_data, ensure_ascii=False)
        return ignore_data
            ...
            if _data.get(ignore) is None:
                ...
                for key, value in _data.items():
                    if value is None or value == '':
                        ...
                        # params_data += (key + "=" + self.transferred_meaning(value) + "&")
                url = self.__yaml_case.url + params_data[:-1]
            else:
                url = url + "/" + _data.get(ignore)
        _headers = self.check_headers_str_null(headers)
        res = requests.request(
            method=method,
    ...
    def request_type_for_data(
        ...
        传参方式判断,    data =  {key: value}  
        存在ignore ,   data = "str"
        存在ignore_list ,   data = "["string"]"
        """
        data = self.__yaml_case.data
        cache_data = cache_regular(str(data))
        data = ast.literal_eval(cache_data)
        if data.get(ignore):
            # 字符串序列化 '"string"'
            data = str(data.get(ignore)).encode("utf-8")

        elif data.get(ignore_list):
            # 字符序列化为json "[\"string\"]"
            data = self.transferred_meaning(data.get(ignore_list))
    ...
    def _sql_data_handler(cls, sql_data, res):
        ...
        if config.env.mysql_db.switch and sql_data is not None:
