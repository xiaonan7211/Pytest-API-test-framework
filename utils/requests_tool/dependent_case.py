...
class DependentCase:
            ...
            try:
                for dependence_case_data in _dependence_case_dates:
                    ...
                    if _case_id == 'self':
                        ...
                        re_data = regular(str(self.get_cache(_case_id)), _case_id)
    ...
    def get_dependent_data(self) -> None:
        ...
        # 兼容 data参数类型 [ {key:value} ]
        if isinstance(yaml_case.data, list):
            yaml_case.data = yaml_case.data[0]
            state = False
        ...
        if _dependent_data is not None and _dependent_data is not False:
            ...
            for key, value in _dependent_data.items():
                ...
                if not isinstance(value, int):
                    list_new_data = list(_new_data)
                    # 兼容整数与字符串的参数替换
                    __index = list_new_data.index("=") + 1 + 1
                    list_new_data.insert(__index, '"""')
                    list_new_data.insert(len(list_new_data), '"""')
                    _new_data = "".join(list_new_data)
                exec(_new_data)
        if not state:
            yaml_case.data = [yaml_case.data]


if __name__ == '__main__':
    a = 'awdadawdawd["adwadwadawd"] = adwadwafefef'
    c = list(a)
    v = c.index("=")
    c.insert(v+1+1, '"""')
    c.insert(len(c), '"""')
