...
class Assert:
    ...
    def sql_switch_handle(
        ...
        """
        # 判断数据库为开关为关闭状态
        if config.env.mysql_db.switch is False:
        ...
        if config.env.mysql_db.switch:
            ...
            if sql_data != {'sql': None}:
                ...
                res_sql_data = self.res_sql_data_bytes(res_sql_data)
                name = AssertMethod(self.assert_data[key]['type']).name
                # 判断sql_data数据长度，走不同断言
                if len(res_sql_data) == 1:
                    res_sql_data = res_sql_data[0]
                self.functions_mapping[name](resp_data, res_sql_data, str(message))
